...
master_host = os.environ.get('ODOO_SESSION_MASTER_REDIS_HOST', 'localhost')
master_port = int(os.environ.get('ODOO_SESSION_MASTER_REDIS_PORT', 6379))
master_password = os.environ.get('ODOO_SESSION_MASTER_REDIS_PASSWORD')

@lazy_property
def session_store(self):
    def writer_for(redis_client):
        if isinstance(redis_client, Sentinel):
            return redis_client
        rep = redis_client.info('replication')
        if rep['role'] == 'slave':
            return redis.Redis(host=master_host, port=master_port, password=master_password)
        else:
            return redis_client
    if sentinel_host:
    ...
    return RedisSessionStore(redis_read=redis_client, redis_write=writer_for(redis_client), prefix=prefix,
