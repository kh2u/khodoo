...
class RedisSessionStore(SessionStore):
    ...
    def __init__(self, redis_read, redis_write, session_class=None,
    ...
    def save(self, session):
        ...
        if self.redis_write.set(key, data):
            return self.redis_write.expire(key, expiration)

    def delete(self, session):
        key = self.build_key(session.sid)
        _logger.debug('deleting session with key %s', key)
        return self.redis_write.delete(key)
    ...
    def list(self):
        keys = self.redis_read.keys('%s*' % self.prefix)
